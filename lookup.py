...
async def lookup(state: AgentState) -> dict:
    ...
    # 1. Check current shortlist first (instant, no DB query)
    shortlist = state.get("shortlist", [])
    if shortlist:
        lookup_result = _find_in_shortlist(company_name, shortlist)

    # 2. Direct ILIKE search
    if not lookup_result:
        rows = await pool.fetch(WEBSITE_FUZZY_MATCH, company_name)
        if rows:
            lookup_result = dict(rows[0])

    # 3. Trigram similarity fallback (handles typos, missing accents/apostrophes)
    if not lookup_result:
        rows = await pool.fetch(WEBSITE_TRIGRAM_MATCH, company_name)
        if rows:
            lookup_result = dict(rows[0])

    # 4. LLM normalization fallback (translate/normalize the name)
    if not lookup_result:
        try:
    ...
    # Enrich with actes + legal info before returning
    if lookup_result:
...
def _find_in_shortlist(name: str, shortlist: list[dict]) -> dict | None:
    ...
    # Exact match first
    for r in shortlist:
        if (r.get("company_name") or "").lower() == name_lower:
            return r

    # Partial match (name is contained in company_name or vice versa)
    for r in shortlist:
